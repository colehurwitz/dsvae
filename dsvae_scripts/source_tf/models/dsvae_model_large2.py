...
class DSVAEL(nn.Module):
    def __init__(self, z_dim, device=None):
        super(DSVAEL, self).__init__()
        self.z_dim = z_dim
        if device is None:
            self.cuda = False
            self.device = None
...
#         weights_init(self.fc1)
        ...
        self.fc_mean = nn.Linear(512*4*4, z_dim)
        weights_init(self.fc_mean)
        self.fc_var = nn.Linear(512*4*4, z_dim)
        weights_init(self.fc_var)

        #DECODER
        self.d1 = nn.Conv2d(3, 64, kernel_size=4, stride=2, padding=1, bias=True, padding_mode='zeros') #[b, 64, 32, 32]
        weights_init(self.d1)

        self.d2 = nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1, bias=True, padding_mode='zeros') #[b, 128, 16, 16]
        ...
        self.d3 = nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1, bias=True, padding_mode='zeros') #[b, 64, 8, 8]
        ...
        self.d4 = nn.Conv2d(256, 512, kernel_size=4, stride=2, padding=1, bias=True, padding_mode='zeros') #[b, 64, 4, 4]
        ...
        self.d8 = nn.ConvTranspose2d(64, 3, kernel_size=4, stride=2, padding=1) #[b, 3, 64, 64]
...
#         h = self.leakyrelu(self.instance_norm_e1(self.e1(x-y)))   #[b, 64, 32, 32]
        ...
        h = h.view(-1,512*4*4)                                    #[b, 512, 4, 4]
#         h = self.leakyrelu(self.fc1(h.view(-1,512*4*4)))          #[b, 512*4*4]
    ...
    def decode(self, y, z):
        ...
        mu = self.mu2(z).reshape(-1, 128, 16, 16)
        sig = self.sig2(z).reshape(-1, 128, 16, 16)
        h = self.leakyrelu(sig*self.instance_norm_d2(self.d2(h)) + mu) #[b, 128, 16, 16]
        
        mu = self.mu3(z).reshape(-1, 256, 8, 8)
        sig = self.sig3(z).reshape(-1, 256, 8, 8)
        h = self.leakyrelu(sig*self.instance_norm_d3(self.d3(h)) + mu) #[b, 256, 8, 8]
        
        mu = self.mu4(z).reshape(-1, 512, 4, 4)
        sig = self.sig4(z).reshape(-1, 512, 4, 4)
        h = self.leakyrelu(sig*self.instance_norm_d4(self.d4(h)) + mu) #[b, 512, 4, 4]
...
#         h = self.leakyrelu(sig*self.instance_norm_d5(self.d5(h)) + mu) #[b, 256, 8, 8]
        h = self.relu(sig*self.instance_norm_d5(self.d5(h)) + mu) #[b, 256, 8, 8]
...
#         h = self.leakyrelu(sig*self.instance_norm_d6(self.d6(h)) + mu) #[b, 128, 16, 16]
        h = self.relu(sig*self.instance_norm_d6(self.d6(h)) + mu) #[b, 128, 16, 16]
...
#         h = self.leakyrelu(sig*self.instance_norm_d7(self.d7(h)) + mu) #[b, 64, 32, 32]
        h = self.relu(sig*self.instance_norm_d7(self.d7(h)) + mu) #[b, 64, 32, 32]
