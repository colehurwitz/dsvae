...
def conv_encoder(input_tensor, num_latent, is_training=True):
  ...
  e5 = tf.layers.dense(flat_e4, 8192, activation=tf.nn.relu, name="e5")
...
def deconv_decoder(latent_tensor, output_shape, is_training=True):
  ...
  """
  del is_training
  d2 = tf.layers.dense(latent_tensor, 8192, activation=tf.nn.relu)
#   d2 = tf.layers.dense(d1, 1024, activation=tf.nn.relu)
  d2_reshaped = tf.reshape(d2, shape=[-1, 4, 4, 64*8])
